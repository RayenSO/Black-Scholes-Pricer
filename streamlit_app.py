...
S = st.sidebar.number_input("Prix du sous-jacent (S, en €)", min_value=1.0, value=100.0, step=1.0)
...
df_params = pd.DataFrame({
    ...
    "Valeur": [f"{str(S)} €", f"{str(K)} €", echeance_affichage, f"{r*100:.2f}%", f"{sigma*100:.2f}%"]
})
...
# Pied de page
st.markdown("---")
st.caption("📌 Développé avec ❤️ par Rayen & Elyasi | Modèle Black-Scholes pour le pricing des options")
